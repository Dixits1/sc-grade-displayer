chrome.storage.local.get(['disabled'], function(data) {
  if (!chrome.runtime.lastError && data.disabled) return;
  ...
  for(var i = 0; i < courses.length; i++)
  {
    ...
    {
      var gpa = value(c.course_title, calculate_grade_clean(c));
      all_gpa.push(gpa);
      display_changes(c, SHOW_OVERALL_GRADE, gpa);
  ...
  var average_gpa = roundTo(all_gpa.reduce(function(aggregate, thisValue) {
    return aggregate + thisValue;
  }) / all_gpa.length, 2);

  var title = document.getElementById("center-top").children[0]; // firstChild doesn't work for some reason
  title.innerHTML = title.innerHTML + " (GPA: " + average_gpa + ")"
  ...
  function display_changes(c, show_grade, extra)
  {
    if(show_grade)
    {
      ...
      var final_displayed_grade = '<span class="added-text">&emsp;(' + letter_final_grade + ", " + raw_final_grade + "%" + (extra ? ", "+extra : "") + ")</span>";
  ...
  function value(title, grade) {
    ...
    for(var i = 0; i < ranges.length; i++)
    {
      if(grade >= ranges[i])
      {
            ...
            // Following shouldn't matter because GPA for a semester course isn't half
