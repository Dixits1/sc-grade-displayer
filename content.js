[...document.querySelectorAll(".td-content-wrapper")].forEach(
  (tdContentWrapperElement) => {
    const awardedGradeElement = [...tdContentWrapperElement.children].find(
      (element) => element.className === "awarded-grade"
    );
    if (awardedGradeElement) {
      const roundedGradeElement = [...awardedGradeElement.children].find(
        (element) =>
          element.className.includes("rounded-grade") ||
          element.className.includes("rubric-grade-value")
      );
      if (roundedGradeElement) {
        roundedGradeElement.removeAttribute("title");
        const roundedGrade = parseFloat(roundedGradeElement.textContent);
        const maxGradeElement = [...tdContentWrapperElement.children].find(
          (element) => element.className === "max-grade"
        );
        if (maxGradeElement) {
          maxGradeElement.removeAttribute("title");
          const maxGrade = parseFloat(
            maxGradeElement.textContent.split(" / ")[1]
          );
          tdContentWrapperElement.setAttribute(
            "data-tooltip",
            `${roundTo((roundedGrade / maxGrade) * 100, 2)}%`
          );
        }
      }
    }
...
var inner_text =
  '<div id="popup"> <div id="close-btn">' +
  svg_text +
  '</div><div id="popup-header-cont"><span id="popup-main-header">Finals Grade Calculator</span><select id="popup-dropdown-header"></select></div><div id="popup-final-grade-cont"><span id="popup-needed-grade">--.-%</span></div><div id="popup-body-cont"><span id="popup-error">Error</span><input id="popup-desired-input" class="popup-input" type="number" min="0" max="100" placeholder="Desired Overall Grade for Course(e.g. 93.5)"/><input id="popup-weight-input" class="popup-input" type="number" min="0" max="100" placeholder="Final Weight(e.g. 20.0)"/></div><div id="popup-footer-cont"><button id="popup-calc-btn">Calculate Grade Needed on Final</button></div></div><div id="popup-mask"></div>';
...
document
  .getElementById("popup-dropdown-header")
  .addEventListener("change", function () {
    ...
    var course_arr_val = this.options[this.selectedIndex].getAttribute(
      "coursearr"
    );
        ...
        if (!isNaN(d) && !isNaN(w)) {
          if (!(d == 0 || w == 0)) {
            if (d <= 100 && w <= 100) {
              ...
              if (final_grade > 1.1) {
                document.getElementById("popup-needed-grade").innerHTML =
                  "...Let's just say you need some major extra credit.";
              } else {
                document.getElementById(
                  "popup-needed-grade"
                ).innerHTML = percent_str;
              }
            } else {
...
for (var i = 0; i < courses.length; i++) {
  ...
  var course_grades = courses[i].getElementsByClassName(
  ...
  if (course_title.length != 0) {
    course_title = course_title[0].firstChild.innerHTML
      .split(">")[2]
      .split("<")[0];
  } else {
  ...
  for (var j = 0; j < course_inners.length; j++) {
    //filtering
    if (
      course_inners[j].getAttribute("data-parent-id") != "" &&
      !course_inners[j].classList.contains("sim-row")
    ) {
      ...
      if (
        course_inner_title != null &&
        course_inner_title.includes("(no grading period)") &&
        course_inners[j].classList.contains("period-row")
      ) {
        break;
      }

      //grade weight text
      category_weight = course_inners[j].getElementsByClassName(
        "percentage-contrib"
      );
      ...
      if (
        course_inners[j].classList.contains("item-row") &&
        !course_inners[j].classList.contains("dropped")
      ) {
        ...
        if (grade_given != -1) {
          ...
          grade_date = grade_date.slice(
            grade_date.indexOf("</span>") + 7,
            grade_date.length
          );
        ...
        if (course_inners[j].classList.contains("last-row-of-tier")) {
          ...
          duplicate_elem.setAttribute(
            "class",
            "report-row item-row last-row-of-tier sim-row"
          );
          duplicate_elem.setAttribute(
            "data-parent-id",
            course_inners[j].getAttribute("data-parent-id")
          ...
          duplicate_elem
            .getElementsByClassName("sim-add-btn")[0]
            .addEventListener("click", function () {
              var prev_elem = this.parentNode.parentNode.parentNode
                .previousSibling;
              var duplicate_elem = this.parentNode.parentNode.parentNode;
              var grade_name_input = duplicate_elem.getElementsByClassName(
                "grade-name-input"
              )[0];
              var given_grade_input = duplicate_elem.getElementsByClassName(
                "given-grade-input"
              )[0];
              var max_grade_input = duplicate_elem.getElementsByClassName(
                "max-grade-input"
              )[0];
              ...
              if (
                given_grade_input_value != "" &&
                max_grade_input_value != ""
              ) {
                ...
                new_elem.setAttribute(
                  "class",
                  "report-row item-row last-row-of-tier sim-grade-row"
                );
                var new_elem_text =
                  '<th scope="row" class="title-column"><div class="reportSpacer-3"><div class="td-content-wrapper"><span class="title">' +
                  grade_name_input_value +
                  '</span> </div></div></th><td class="grade-column"><div class="td-content-wrapper"><span class="awarded-grade"><span class="rounded-grade">' +
                  given_grade_input_value +
                  '</span></span><span class="max-grade"> / ' +
                  max_grade_input_value +
                  '</span><div class="grade-wrapper"></div></div></td><td class="comment-column"><div class="td-content-wrapper"><a class="sim-remove-btn" given="' +
                  given_grade_input_value +
                  '" max="' +
                  max_grade_input_value +
                  '" arrpos="' +
                  grade_arr_length +
                  '">-&emsp;Remove</a><span class="visually-hidden">No comment</span></div></td>';
                ...
                new_elem
                  .getElementsByClassName("sim-remove-btn")[0]
                  .addEventListener("click", function () {
                    var given = parseFloat(this.getAttribute("given"));
                    var max = parseFloat(this.getAttribute("max"));

                    data_parent_id = this.parentNode.parentNode.parentNode.previousSibling.getAttribute(
                      "data-parent-id"
                    );
                ...
                cat_obj.add_grade(
                  parseFloat(given_grade_input_value),
                  parseFloat(max_grade_input_value),
                  "",
                  grade_name_input_value,
                  new_elem
                );
        ...
        if (category_weight.charAt(0) == "(") {
          ...
          cat_weight =
            parseFloat(category_weight.split("(")[1].split("%")[0]) / 100;
...
function init_popup() {
  ...
  var course_arr_val = popup_dd.options[popup_dd.selectedIndex].getAttribute(
    "coursearr"
  );
  popup_selected_course = course_array[course_arr_val];
}
...
function display_rogerhub(c) {
  ...
  if (course_elem.length == 1) {
    ...
    rh_button.addEventListener(
      "click",
      function (event) {
        var gradebook_cont = parent_elem.getElementsByClassName(
          "gradebook-course-grades"
        )[0];
...
function display_changes(c, show_grade) {
  if (show_grade) {
    ...
    var final_displayed_grade =
      '<span class="added-text">&emsp;(' +
      letter_final_grade +
      ", " +
      raw_final_grade +
      "%" +
      ")</span>";
    c.course_elem.firstChild.firstChild.innerHTML =
      c.course_elem.firstChild.firstChild.innerHTML + final_displayed_grade;
  }
  ...
  for (var cat of c.cats) {
    if (cat.elem != null) {
      ...
      if (d.length == 1 && !cat.is_empty && cat.weight != null) {
        ...
        d[0].innerHTML =
          inner_val + '<span class="added-text">&emsp;(' + p + "%)</span>";
      }
    }
  }
}
...
function populate_dropdown() {
  ...
  for (var i = 0; i < course_array.length; i++) {
    ...
    if (!title.includes("Advisory")) dd_elem.appendChild(new_option);
  }
}
